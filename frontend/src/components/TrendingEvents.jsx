...
const TrendingEvents = () => {
  ...
  // Fetch events and sort by priority
  useEffect(() => {
  ...
  const getPriority = (eventTypes) => {
    if (!Array.isArray(eventTypes)) return 100;
    if (eventTypes.includes("Trending")) return 1;
    if (eventTypes.includes("Hot")) return 2;
    if (eventTypes.includes("Mostbooked")) return 3;
    return 100;
  };
    ...
    <section className="py-12 bg-gray-100">
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
          {events.map((event) => {
            // Ensure location exists and extract latitude/longitude
